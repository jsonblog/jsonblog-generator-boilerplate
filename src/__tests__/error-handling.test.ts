...
describe('Error Handling', () => {
  ...
  test('should throw error when blog config is missing', async () => {
    await expect(generator(null as any, basePath)).rejects.toThrow('Blog configuration is required');
  });
  ...
  test('should throw error when site config is missing', async () => {
    ...
    await expect(generator(invalidBlog, basePath)).rejects.toThrow('Blog site configuration with title is required');
  });
  ...
  test('should throw error when basics config is missing', async () => {
    ...
    await expect(generator(invalidBlog, basePath)).rejects.toThrow('Blog basics configuration with author name is required');
  });
  ...
  test('should continue processing when one post fails', async () => {
    ...
    expect(files.filter(f => f.name.endsWith('.html') && f.name !== 'index.html' && !f.name.startsWith('page/')).length).toBe(3);
