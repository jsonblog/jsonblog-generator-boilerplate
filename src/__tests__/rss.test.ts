...
describe('RSS Feed Generation', () => {
  const mockBlog: Blog = {
    ...
    posts: [
      ...
      {
        title: 'Second Post',
        content: '<p>This is a longer post without a description. It contains multiple paragraphs and should be truncated in the RSS feed. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>',
  ...
  test('should include blog metadata in RSS', async () => {
    ...
    expect(rssFile?.content).toContain('<description><![CDATA[A test blog for RSS]]></description>');
  ...
  test('should include posts in RSS feed', async () => {
    ...
    expect(rssFile?.content).toContain('<description><![CDATA[This is the first post]]></description>');
