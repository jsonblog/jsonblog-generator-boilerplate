...
describe('Sitemap Generation', () => {
  ...
  test('should generate sitemap.xml file', async () => {
    ...
    expect(sitemapFile?.content).toContain('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">');
  });
  ...
  test('should set appropriate changefreq values', async () => {
    ...
    const homepageUrl = sitemapFile?.content.match(/<url>[\s\S]*?<loc>https:\/\/testblog\.com\/<\/loc>[\s\S]*?<\/url>/);
